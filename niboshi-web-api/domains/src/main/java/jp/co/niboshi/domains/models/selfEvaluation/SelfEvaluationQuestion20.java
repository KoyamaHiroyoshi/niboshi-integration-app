...
public class SelfEvaluationQuestion20 implements NiboshiModel {
  private String inner;

  public SelfEvaluationQuestion20(String selfEvaluationQuestion20) {
    if (Objects.isNull(selfEvaluationQuestion20)) {
      this.throwDomainModelException(
          String.format(ErrorMessages.NULL.getMessage(), this.getModelName()));
    } else if (selfEvaluationQuestion20.isEmpty()) {
      this.throwDomainModelException(
          String.format(ErrorMessages.EMPTY.getMessage(), this.getModelName()));
    }

    this.inner = selfEvaluationQuestion20;
  }
