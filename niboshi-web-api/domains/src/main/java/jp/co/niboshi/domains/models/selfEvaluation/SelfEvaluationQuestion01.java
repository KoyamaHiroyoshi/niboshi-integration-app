...
public class SelfEvaluationQuestion01 implements NiboshiModel {
  private String inner;

  public SelfEvaluationQuestion01(String selfEvaluationQuestion01) {
    if (Objects.isNull(selfEvaluationQuestion01)) {
      this.throwDomainModelException(
          String.format(ErrorMessages.NULL.getMessage(), this.getModelName()));
    } else if (selfEvaluationQuestion01.isEmpty()) {
      this.throwDomainModelException(
          String.format(ErrorMessages.EMPTY.getMessage(), this.getModelName()));
    }

    this.inner = selfEvaluationQuestion01;
  }
