...
public class SelfEvaluationQuestion45 implements NiboshiModel {
  private String inner;

  public SelfEvaluationQuestion45(String selfEvaluationQuestion45) {
    if (Objects.isNull(selfEvaluationQuestion45)) {
      this.throwDomainModelException(
          String.format(ErrorMessages.NULL.getMessage(), this.getModelName()));
    } else if (selfEvaluationQuestion45.isEmpty()) {
      this.throwDomainModelException(
          String.format(ErrorMessages.EMPTY.getMessage(), this.getModelName()));
    }

    this.inner = selfEvaluationQuestion45;
  }
