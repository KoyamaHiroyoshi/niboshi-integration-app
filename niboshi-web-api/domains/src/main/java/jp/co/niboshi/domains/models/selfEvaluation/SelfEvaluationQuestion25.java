...
public class SelfEvaluationQuestion25 implements NiboshiModel {
  private String inner;

  public SelfEvaluationQuestion25(String selfEvaluationQuestion25) {
    if (Objects.isNull(selfEvaluationQuestion25)) {
      this.throwDomainModelException(
          String.format(ErrorMessages.NULL.getMessage(), this.getModelName()));
    } else if (selfEvaluationQuestion25.isEmpty()) {
      this.throwDomainModelException(
          String.format(ErrorMessages.EMPTY.getMessage(), this.getModelName()));
    }

    this.inner = selfEvaluationQuestion25;
  }
