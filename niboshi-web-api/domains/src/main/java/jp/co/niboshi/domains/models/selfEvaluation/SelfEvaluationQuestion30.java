...
public class SelfEvaluationQuestion30 implements NiboshiModel {
  private String inner;

  public SelfEvaluationQuestion30(String selfEvaluationQuestion30) {
    if (Objects.isNull(selfEvaluationQuestion30)) {
      this.throwDomainModelException(
          String.format(ErrorMessages.NULL.getMessage(), this.getModelName()));
    } else if (selfEvaluationQuestion30.isEmpty()) {
      this.throwDomainModelException(
          String.format(ErrorMessages.EMPTY.getMessage(), this.getModelName()));
    }

    this.inner = selfEvaluationQuestion30;
  }
