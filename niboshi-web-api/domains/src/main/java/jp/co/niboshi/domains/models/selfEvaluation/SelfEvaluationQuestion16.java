...
public class SelfEvaluationQuestion16 implements NiboshiModel {
  private String inner;

  public SelfEvaluationQuestion16(String selfEvaluationQuestion16) {
    if (Objects.isNull(selfEvaluationQuestion16)) {
      this.throwDomainModelException(
          String.format(ErrorMessages.NULL.getMessage(), this.getModelName()));
    } else if (selfEvaluationQuestion16.isEmpty()) {
      this.throwDomainModelException(
          String.format(ErrorMessages.EMPTY.getMessage(), this.getModelName()));
    }

    this.inner = selfEvaluationQuestion16;
  }
