...
public class SelfEvaluationQuestion39 implements NiboshiModel {
  private String inner;

  public SelfEvaluationQuestion39(String selfEvaluationQuestion39) {
    if (Objects.isNull(selfEvaluationQuestion39)) {
      this.throwDomainModelException(
          String.format(ErrorMessages.NULL.getMessage(), this.getModelName()));
    } else if (selfEvaluationQuestion39.isEmpty()) {
      this.throwDomainModelException(
          String.format(ErrorMessages.EMPTY.getMessage(), this.getModelName()));
    }

    this.inner = selfEvaluationQuestion39;
  }
